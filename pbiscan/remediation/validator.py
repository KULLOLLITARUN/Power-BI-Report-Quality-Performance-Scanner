...
class SandboxValidator:
    ...
    def apply_patches_to_dir(cls, patches: list[Patch], target_dir: Path) -> list[str]:
        ...
        for patch in patches:
            # Locate file in target_dir
            # patch.file_path may be absolute from original model or relative
            rel_path = patch.file_path
            # Find matching relative path inside target_dir
            candidates = [p for p in target_dir.glob(f"**/{patch.file_path.name}")]
            if not candidates:
                errors.append(f"Target file not found in sandbox: {patch.file_path.name}")
                continue
            ...
            if current_hash != patch.source_hash:
                errors.append(
                    ...
                    f"expected {patch.source_hash[:8]}, got {current_hash[:8]}"
                )
                continue
            ...
            # Apply chunks in reverse line order to prevent offset shifts
            sorted_chunks = sorted(patch.chunks, key=lambda c: c.start_line, reverse=True)
            chunk_error = False
            ...
            for chunk in sorted_chunks:
                ...
                # Replace slice
                lines[start_idx:end_idx] = [chunk.replacement_text]
    ...
    def validate_plan(
        ...
        with tempfile.TemporaryDirectory(prefix="pbiscan_sandbox_") as temp_dir:
            ...
            # Check: Did target findings resolve?
            if not target_rules.issubset(resolved_rules):
                missing_rules = target_rules - resolved_rules
                rejection_reasons.append(
                    f"Target finding was not resolved after patch: {', '.join(missing_rules)}"
                )
